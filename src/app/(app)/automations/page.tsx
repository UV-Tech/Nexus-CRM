    ...
    <div className="p-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            בנו תהליכים שרצים כאשר לידים נכנסים או משתנים. ארגנו אותם בתיקיות.
          </p>
        </div>
          ...
          <section key={group.folder?.id ?? "none"}>
            <div className="flex items-center justify-between">
              <h2 className="flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-slate-500">
                {group.folder ? `📁 ${group.folder.name}` : "ללא תיקייה"}
      ...
      <section className="mt-10">
        ...
        <div className="mt-2 overflow-hidden rounded-xl border border-slate-200 bg-white">
          {recentRuns.map((r) => (
            ...
            >
              <span className="text-slate-700">
                {nameById.get(r.automation_id) ?? "אוטומציה"}
          ...
          {recentRuns.length === 0 && (
            <p className="px-5 py-6 text-center text-sm text-slate-400">
              אין הרצות עדיין. הפעילו אוטומציה והיא תופיע כאן.
            </p>
          )}
        </div>
      </section>
